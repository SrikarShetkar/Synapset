...
import { BrainCircuit, Play, Award, Zap } from "lucide-react";
...
export default function BrainBreak() {
  ...
  const finishChallenge = () => {
    const finalScore = Math.floor(Math.random() * 50 + 50); // Mock score 50-100
    setScore(finalScore);
    setShowBadge(true);
      ...
      { data: { blinkScore: finalScore } },
      {
        onSuccess: () => {
          ...
          toast({
            ...
            description: `You scored ${finalScore} on blink sync. Neural pathways cleared.`,
          });
        }
      }
    );
    ...
    <Layout>
      <div className="p-8 max-w-3xl mx-auto w-full min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center">
        <div className="text-center mb-12">
          <h1 className="text-3xl font-bold text-foreground">Synapse Reset</h1>
          <p className="text-muted-foreground mt-2 max-w-md mx-auto">
            Clear cognitive buildup. Sync your breathing and blinking with the pulse.
          </p>
        </div>
          ...
          <AnimatePresence mode="wait">
            {!isActive && !showBadge ? (
              ...
              >
                ...
                {/* Core pulse */}
                <div className={`w-48 h-48 rounded-full flex flex-col items-center justify-center transition-all duration-300 ${
                  pulsing ? 'bg-primary scale-110 shadow-[0_0_50px_hsl(var(--primary))]' : 'bg-primary/20 scale-100'
                }`}>
                  <span className={`text-5xl font-mono font-bold ${pulsing ? 'text-primary-foreground' : 'text-primary'}`}>
                    {timeLeft}
                  </span>
                </div>
              </motion.div>
              ...
              >
                ...
                <div className="text-sm font-bold text-muted-foreground tracking-widest uppercase mb-1">Score</div>
                <div className="text-6xl font-black text-foreground font-mono">{score}</div>
                <Button variant="outline" className="mt-8 rounded-full" onClick={startChallenge}>
